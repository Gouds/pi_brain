...
export default function ServoSection() {
  ...
  useEffect(() => {
    if (!activeProfile?.id || !sectionId) return
    profileAdminGetServos(activeProfile.id)
      .then(all => {
        // Match on section field; fall back to bus name for servos without a section set
        const filtered = (Array.isArray(all) ? all : [])
          .filter(s => (s.section || s.bus) === sectionId)
        setServos(filtered)
        const init = {}
    ...
    <div style={{ padding: '10px 12px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
      ...
      {servos.length === 0 ? (
        <p style={{ color: 'var(--text-secondary)' }}>
          No servos in this section. Go to{' '}
          <a href="#/admin/servos" style={{ color: 'var(--accent)' }}>Admin → Servos</a>
          {' '}and set the <strong>section</strong> field to <strong>{sectionId}</strong>.
        </p>
      ) : (
          ...
          >
            ...
            {visible.map((servo, i) => (
              <ServoCard
                ...
                servo={servo}
                pos={positions[servo.name] ?? servo.open_position ?? 0}
