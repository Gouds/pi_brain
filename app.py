...
def _save_arduino_config(config):
    os.makedirs("configs", exist_ok=True)
    with open(ARDUINO_CONFIG_PATH, "w") as f:
        json.dump(config, f, indent=4)

def _generate_sketch(config):
    """Substitute template placeholders with values from arduino_config and write controller.ino."""
...
@app.get("/admin/arduino/config", tags=["Arduino"])
async def arduino_get_config():
    return _load_arduino_config()

@app.put("/admin/arduino/config", tags=["Arduino"])
async def arduino_save_config(config: dict):
    _save_arduino_config(config)
    _generate_sketch(config)
    return config

@app.get("/admin/arduino/flash", tags=["Arduino"])
async def arduino_flash():
    """
    Compile and upload the Arduino sketch using arduino-cli.
    Streams output line-by-line as Server-Sent Events.
    Requires arduino-cli to be installed and arduino:avr core to be set up.
    """
    from fastapi.responses import StreamingResponse
    import shutil

    config = _load_arduino_config()
    _generate_sketch(config)
    ...
    if not shutil.which("arduino-cli"):
        async def err():
            yield "data: ERROR: arduino-cli not found. Install it with:\n\n"
            yield "data: curl -fsSL https://raw.githubusercontent.com/arduino/arduino-cli/master/install.sh | sh\n\n"
            yield "data: Then run: arduino-cli core install arduino:avr\n\n"
            yield "event: done\ndata: error\n\n"
